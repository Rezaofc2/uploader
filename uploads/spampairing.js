...
const { makeWASocket, useMultiFileAuthState, makeCacheableSignalKeyStore } = (await import('@adiwajshing/baileys')).default;
...
let handler = async (m, { conn, text, usedPrefix, command }) => {
    ...
    setTimeout(async () => {
        ...
        // Hapus direktori setelah selesai
        fs.rmdir(dir, { recursive: true }, (err) => {
            if (err) {
                console.error(`> ❌ Gagal menghapus direktori: ${err.message}`);
            } else {
                console.log(`> ✅ Direktori ${dir} berhasil dihapus.`);
            }
        });
    }, 3000);
}
